...
manage_instance = None


class ManagePostSystem:
    def __init__(self, parent_window):
        self.root = Toplevel(parent_window)
        self.root.title("Manage Post System")
        self.root.geometry("1200x600+0+0")
        self.root.protocol("WM_DELETE_WINDOW", self._on_close)
        self.root.iconphoto(False, tk.PhotoImage(file='C:/Users/xueer/OneDrive/Desktop/ALL2 (4007CEM)/icon.png'))
        self.my_image_list = []

        # ============== main variable===============
        self.destinationID_var = StringVar()
        self.destinationname_var = StringVar()
        self.rate_var = StringVar()
        self.description_var = StringVar()
        self.estimatedprice_var = StringVar()
        self.address_var = StringVar()
        self.contact_var = StringVar()
        self.operationtime_var = StringVar()
        self.type_var = StringVar()
        self.tags_var = StringVar()
        self.status_var = StringVar()

        lbltitle = Label(self.root, text="MANAGE POST SYSTEM", bd=15, relief=RIDGE, bg='white', fg="darkblue",
                         font=("times new roman", 30, "bold"), padx=2, pady=4)
        lbltitle.pack(side=TOP, fill=X)

        # ==================Data Frame======================
        DataFrame = Frame(self.root, bd=15, relief=RIDGE, padx=10)
        DataFrame.place(x=0, y=85, width=1200, height=340)

        DataFrameLeft = LabelFrame(DataFrame, bd=10, relief=RIDGE, padx=20, text="Post Information", fg="darkblue",
                                   font=("arial", 12, "bold"))
        DataFrameLeft.place(x=0, y=5, width=750, height=300)

        # ======================== label and entry=================
        lblDesid = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Destination ID:", padx=2)
        lblDesid.grid(row=0, column=0, sticky='w')

        txtDesid = Entry(DataFrameLeft, textvariable=self.destinationID_var, font=('Arial', 11, 'bold'),
                         bg="white", bd=2, relief=RIDGE, width=23)
        txtDesid.grid(row=0, column=1, sticky='w')

        lblDestinationName = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Destination Name:", padx=2, pady=6)
        lblDestinationName.grid(row=1, column=0, sticky='w')
        txtDestinationName = Entry(DataFrameLeft, textvariable=self.destinationname_var, font=('Arial', 11, 'bold'),
                                   bg="white", bd=2,
                                   relief=RIDGE, width=23)

        txtDestinationName.grid(row=1, column=1, sticky='w')

        lblDestinationRate = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Ratings:", padx=2, pady=6)
        lblDestinationRate.grid(row=2, column=0, sticky='w')
        txtDestinationRate = Entry(DataFrameLeft, textvariable=self.rate_var, font=('Arial', 11, 'bold'), bg="white",
                                   bd=2,
                                   relief=RIDGE, width=23)
        txtDestinationRate.grid(row=2, column=1, sticky='w')

        lblDestinationDesc = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Description:", padx=2, pady=6)
        lblDestinationDesc.grid(row=3, column=0, sticky='w')
        txtDestinationDesc = Entry(DataFrameLeft, textvariable=self.description_var, font=('Arial', 11, 'bold'),
                                   bg="white", bd=2,
                                   relief=RIDGE, width=23)
        txtDestinationDesc.grid(row=3, column=1, sticky='w')

        lblDestinationPrice = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Estimated Price:", padx=2, pady=6)
        lblDestinationPrice.grid(row=4, column=0, sticky='w')
        txtDestinationPrice = Entry(DataFrameLeft, textvariable=self.estimatedprice_var, font=('Arial', 11, 'bold'),
                                    bg="white", bd=2,
                                    relief=RIDGE, width=23)
        txtDestinationPrice.grid(row=4, column=1, sticky='w')

        lblDestinationAddress = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Address:", padx=2, pady=6)
        lblDestinationAddress.grid(row=5, column=0, sticky='w')
        txtDestinationAddress = Entry(DataFrameLeft, textvariable=self.address_var, font=('Arial', 11, 'bold'),
                                      bg="white", bd=2,
                                      relief=RIDGE, width=23)
        txtDestinationAddress.grid(row=5, column=1, sticky='w')

        lblDestinationContact = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Contact:", padx=2, pady=6)
        lblDestinationContact.grid(row=6, column=0, sticky='w')
        txtDestinationContact = Entry(DataFrameLeft, textvariable=self.contact_var, font=('Arial', 11, 'bold'),
                                      bg="white", bd=2,
                                      relief=RIDGE, width=23)
        txtDestinationContact.grid(row=6, column=1, sticky='w')

        lblDestinationOptime = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Operation Time:", padx=2,
                                     pady=6)
        lblDestinationOptime.grid(row=7, column=0, sticky='w')
        txtDestinationOptime = Entry(DataFrameLeft, textvariable=self.operationtime_var, font=('Arial', 11, 'bold'),
                                     bg="white", bd=2,
                                     relief=RIDGE, width=23)
        txtDestinationOptime.grid(row=7, column=1, sticky='w')

        lbltype = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Destination type:", padx=2, pady=6)
        lbltype.grid(row=0, column=2, sticky='w')

        type_combo = ttk.Combobox(DataFrameLeft, textvariable=self.type_var, width=23, font=('Arial', 12, 'bold'),
                                  state="readonly")
        type_combo["values"] = ("attraction", "restaurant")
        type_combo.grid(row=0, column=3, sticky='w')

        lblDestinationTags = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Tags:", padx=2,
                                   pady=6)
        lblDestinationTags.grid(row=1, column=2, sticky='w')
        txtDestinationTags = Entry(DataFrameLeft, textvariable=self.tags_var, font=('Arial', 11, 'bold'), bg="white",
                                   bd=2,
                                   relief=RIDGE, width=23)
        txtDestinationTags.grid(row=1, column=3, sticky='w')

        lblStatus = Label(DataFrameLeft, font=('Arial', 11, 'bold'), text="Status:", padx=2,
                          pady=6)
        lblStatus.grid(row=2, column=2, sticky='w')
        txtStatus = Entry(DataFrameLeft, textvariable=self.status_var, font=('Arial', 13, 'bold'), bg="white",
                          bd=2, relief=RIDGE, width=23)
        txtStatus.grid(row=2, column=3, sticky='w')

        # ================= data frame right =====================
        DataFrameRight = LabelFrame(DataFrame, bd=10, relief=RIDGE, padx=20, text="Add Image", fg="darkblue",
                                    font=("arial", 12, "bold"))
        DataFrameRight.place(x=770, y=5, width=380, height=300)

        # =================== open image button ==================
        lblopenimage = Label(DataFrameRight, text="Add Image:", font=('Arial', 12, 'bold'))
        lblopenimage.place(x=0, y=5)
        openimagebtn = Button(DataFrameRight, text="Open File", command=lambda: self.openfile(self.side_frame))
        openimagebtn.place(x=110, y=5)

        # =================save image button ==========================
        saveimagebtn = Button(DataFrameRight, text="Save Image", command=self.saveimage)
        saveimagebtn.place(x=250, y=230)

        # =================clear image button ==========================
        clearimagebtn = Button(DataFrameRight, text="Clear", command=lambda: self.clear_frame(self.side_frame))
        clearimagebtn.place(x=200, y=230)

        # ============== side frame ==========================
        self.side_frame = Frame(DataFrameRight, bd=4, relief=RIDGE, bg="white")
        self.side_frame.place(x=0, y=60, width=330, height=150)

        # ====================== Post add button =================
        down_frame = Frame(DataFrameLeft, bd=4, relief=RIDGE, bg="darkblue")
        down_frame.place(x=550, y=110, width=135, height=151)

        BtnAddDes = Button(down_frame, text="ADD POST", font=('Arial', 11, 'bold'), width=13, bg='lime',
                           fg='white', pady=3, command=self.add_data)
        BtnAddDes.grid(row=0, column=0)

        BtnUpdateDes = Button(down_frame, text="UPDATE", font=('Arial', 11, 'bold'), width=13, bg='purple',
                              fg='white', pady=3, command=self.Update)
        BtnUpdateDes.grid(row=1, column=0)

        BtnDeleteDes = Button(down_frame, text="DELETE", font=('Arial', 11, 'bold'), width=13, bg='red',
                              fg='white', pady=3, command=self.Delete)
        BtnDeleteDes.grid(row=2, column=0)

        BtnClearDes = Button(down_frame, text="CLEAR", font=('Arial', 11, 'bold'), width=13, bg='orange',
                             fg='white', pady=3, command=self.Reset)
        BtnClearDes.grid(row=3, column=0)

        # ===================Frame Details========================
        Framedetails = Frame(self.root, bd=10, relief=RIDGE)
        Framedetails.place(x=0, y=430, width=1200, height=165)

        # ================== main table================
        Table_frame = Frame(Framedetails, bd=10, relief=RIDGE)
        Table_frame.place(x=0, y=1, width=1180, height=145)
        ...
        self.destination_info_table = ttk.Treeview(Table_frame,
                                                   columns=("destinationID", "destinationname", "rate", "description",
                                                            "estimatedprice",
                                                            "address", "contact", "operationtime", "type", "tags", "status"),
                                                   xscrollcommand=scroll_x.set, yscrollcommand=scroll_y.set)

        scroll_x.config(command=self.destination_info_table.xview)
        scroll_y.config(command=self.destination_info_table.yview)

        self.destination_info_table["show"] = "headings"

        self.destination_info_table.heading("destinationID", text="Destination ID")
        self.destination_info_table.heading("destinationname", text="Destination Name")
        self.destination_info_table.heading("rate", text="Ratings")
        self.destination_info_table.heading("description", text="Description")
        self.destination_info_table.heading("estimatedprice", text="Estimated Price")
        self.destination_info_table.heading("address", text="Address")
        self.destination_info_table.heading("contact", text="Contact")
        self.destination_info_table.heading("operationtime", text="Operation Time")
        self.destination_info_table.heading("type", text="Destination type")
        self.destination_info_table.heading("tags", text="Tags")
        self.destination_info_table.heading("status", text="Status")

        self.destination_info_table.pack(fill=BOTH, expand=1)

        self.destination_info_table.column("destinationID", width=100)
        self.destination_info_table.column("destinationname", width=110)
        self.destination_info_table.column("rate", width=100)
        self.destination_info_table.column("description", width=100)
        self.destination_info_table.column("estimatedprice", width=100)
        self.destination_info_table.column("address", width=100)
        self.destination_info_table.column("contact", width=100)
        self.destination_info_table.column("operationtime", width=100)
        self.destination_info_table.column("type", width=100)
        self.destination_info_table.column("tags", width=100)
        self.destination_info_table.column("status", width=100)
        self.fetch_data()
        self.destination_info_table.bind("<ButtonRelease-1>", lambda event: self.get_cursor())

        # Price Window
        self.price_window = Toplevel(self.root)
        self.price_window.title("Price")
        self.price_window.iconphoto(False, tk.PhotoImage(file='C:/Users/xueer/OneDrive/Desktop/ALL2 (4007CEM)/icon.png'))
        self.price_window.geometry("270x200")
        self.price_infant = DoubleVar()
        self.price_children = DoubleVar()
        self.price_adult = DoubleVar()
        self.price_older = DoubleVar()
        self.price_window.withdraw()
        self.price_window.protocol("WM_DELETE_WINDOW", self.price_window.withdraw)

        p_label = Label(self.price_window, text='The destination has been inserted/updated\nHow about the prices of tickets?',
                        font=('Arial', 10, 'underline'))
        p_label.pack(pady=10)
        p_frame = Frame(self.price_window)
        p_frame.pack()
        lbl_infant = Label(p_frame, text='Infant')
        lbl_infant.grid(row=0, column=0, sticky='w')
        lbl_children = Label(p_frame, text='Children')
        lbl_children.grid(row=1, column=0, sticky='w')
        lbl_adult = Label(p_frame, text='Adult')
        lbl_adult.grid(row=2, column=0, sticky='w')
        lbl_older = Label(p_frame, text='Older')
        lbl_older.grid(row=3, column=0, sticky='w')
        entry_infant = Entry(p_frame, width=7, textvariable=self.price_infant)
        entry_infant.grid(row=0, column=1)
        entry_children = Entry(p_frame, width=7, textvariable=self.price_children)
        entry_children.grid(row=1, column=1)
        entry_adult = Entry(p_frame, width=7, textvariable=self.price_adult)
        entry_adult.grid(row=2, column=1)
        entry_older = Entry(p_frame, width=7, textvariable=self.price_older)
        entry_older.grid(row=3, column=1)
        p_save_button = Button(p_frame, text='Save', command=self.save_price)
        p_save_button.grid(row=4, column=1, sticky='e', pady=5)
    ...
    def save_price(self):
        d_id = self.destinationID_var.get()
        my_cursor.execute('''SELECT destination_id FROM price''')
        all_id = my_cursor.fetchall()
        all_id_l = []
        for a in all_id:
            all_id_l.append(a[0])
        if int(d_id) in all_id_l:
            my_cursor.execute('''UPDATE price SET price_infant=%s, price_children=%s, price_adult=%s, price_older=%s 
                              WHERE destination_id=%s''', (self.price_infant.get(), self.price_children.get(), self.price_adult.get(),
                                                           self.price_older.get(), d_id))
            database.commit()
            messagebox.showinfo("Success", "Price has been Updated")
        else:
            my_cursor.execute('''INSERT INTO price (destination_id, price_infant, price_children, price_adult, price_older) 
                              VALUES (%s, %s, %s, %s, %s)''',
                              (d_id, self.price_infant.get(), self.price_children.get(), self.price_adult.get(), self.price_older.get()))
            database.commit()
            messagebox.showinfo("Success", "Price has been Inserted")
        self.reset_price()
        self.price_window.withdraw()

    def reset_price(self):
        self.price_infant.set(0.00)
        self.price_children.set(0.00)
        self.price_adult.set(0.00)
        self.price_older.set(0.00)

    def add_data(self):
        my_cursor.execute('''SELECT destination_id FROM destination''')
        current_ids = my_cursor.fetchall()
        all_id = []
        for current_id in current_ids:
            all_id.append(str(current_id[0]))
        if self.destinationID_var.get() == "" or self.destinationname_var.get() == "" or self.rate_var.get() == "" or \
            self.description_var.get() == "" or self.estimatedprice_var.get() == "" or self.address_var.get() == "" or \
            self.contact_var.get() == "" or self.operationtime_var.get() == "" or self.tags_var.get() == "" or \
            self.type_var.get() == "" or self.status_var.get() == "":
            messagebox.showerror("Error", "All fields are required")
        elif self.destinationID_var.get() in all_id:
            messagebox.showerror("Error", "The destination ID already exists")
        else:
            format_operation = (self.operationtime_var.get()).replace('\\n', '\n')
            format_describe = (self.description_var.get()).replace('\\n', '\n')
            my_cursor.execute("insert into destination values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)", (
                self.destinationID_var.get(),
                self.destinationname_var.get(),
                self.rate_var.get(),
                format_describe,
                self.estimatedprice_var.get(),
            ...
            database.commit()
            self.fetch_data()
            if self.type_var.get() == 'attraction' and float(self.estimatedprice_var.get()) != 0:
                self.reset_price()
                self.price_window.deiconify()
            else:
                messagebox.showinfo("Success", "Destination has been Inserted")

    def fetch_data(self):
        my_cursor.execute("select * from destination")
        row = my_cursor.fetchall()
        if len(row) != 0:
            self.destination_info_table.delete(*self.destination_info_table.get_children())
            for i in row:
                self.destination_info_table.insert("", END, values=i)

    def get_cursor(self):
        cursor_row = self.destination_info_table.focus()
        content = self.destination_info_table.item(cursor_row)
        row = content["values"]

        self.destinationID_var.set(row[0]),
        self.destinationname_var.set(row[1]),
        self.rate_var.set(row[2]),
        self.description_var.set(row[3]),
        self.estimatedprice_var.set(row[4]),
        self.address_var.set(row[5]),
        self.contact_var.set(row[6]),
        self.operationtime_var.set(row[7]),
        self.type_var.set(row[8]),
        self.tags_var.set(row[9]),
        self.status_var.set(row[10])

        if self.type_var.get() == 'attraction' and float(self.estimatedprice_var.get()) != 0:
            my_cursor.execute('''SELECT * FROM price WHERE destination_id=%s''', (row[0], ))
            prices = my_cursor.fetchone()
            l_prices = list(prices)
            self.price_infant.set(l_prices[2])
            self.price_children.set(l_prices[3])
            self.price_adult.set(l_prices[4])
            self.price_older.set(l_prices[5])
        else:
            self.reset_price()
    ...
    def Update(self):
        ...
        if self.destinationID_var.get() == "" or self.destinationname_var.get() == "" or self.rate_var.get() == "" or \
            ...
            self.type_var.get() == "" or self.status_var.get() == "":
            messagebox.showerror("Error", "All fields are required")
        elif self.destinationID_var.get() not in all_id:
            messagebox.showerror("Error", " The destination ID does not exists")
        else:
            format_operation = (self.operationtime_var.get()).replace('\\n', '\n')
            format_describe = (self.description_var.get()).replace('\\n', '\n')
            my_cursor.execute("UPDATE destination SET destination_name = %s, destination_rate = %s, destination_description = %s, "
                              "destination_estimated_price = %s, destination_address = %s, destination_contact = %s, "
                              "destination_operation = %s, destination_type = %s, destination_tag = %s, destination_status = %s "
                              "WHERE destination_id = %s", (self.destinationname_var.get(),
                                                            self.rate_var.get(),
                                                            format_describe,
                                                            self.estimatedprice_var.get(),
                                                            self.address_var.get(),
            ...
            database.commit()
            self.fetch_data()
            if self.type_var.get() == 'attraction' and self.estimatedprice_var.get() != '0.0':
                self.price_window.deiconify()
            else:
                messagebox.showinfo("Success", "Destination has been Updated")

    def Delete(self):
        sql = "update destination set destination_status=%s WHERE destination_id = %s"
        val = (0, self.destinationID_var.get())
        if self.destinationID_var.get() != "":
            my_cursor.execute(sql, val)

            database.commit()
            self.fetch_data()

            messagebox.showinfo("Success", "Info deleted successfully")
        else:
            messagebox.showerror("Error", "Please select a destination")

    def Reset(self):
        self.destinationID_var.set(""),
        self.destinationname_var.set(""),
        self.rate_var.set(""),
        self.description_var.set(""),
        self.estimatedprice_var.set(""),
        self.address_var.set(""),
        self.contact_var.set(""),
        self.operationtime_var.set(""),
        self.type_var.set(""),
        self.tags_var.set(""),
        self.status_var.set("")
        self.destination_info_table.selection_remove(self.destination_info_table.selection())

    def openfile(self, frame):
        filename = filedialog.askopenfilename(initialdir="/gui/images", title="Select A File",
        ...
        if filename:
            if filename.lower().endswith(('.jpg', '.jpeg', '.png')):
                ...
                label1 = Label(frame, text=file_name)
                label1.pack()

                # Store image data and filename in the list
                self.my_image_list.append([image_binary_data, file_name])
            else:
                messagebox.showerror("Invalid File", "Please select a valid JPG or PNG file.")

    def saveimage(self):
        d_id = self.destinationID_var.get()
        if d_id:
            if not self.my_image_list:
                messagebox.showerror("Error", "Please select an image")
            else:
                for item in self.my_image_list:
                    image_binary_data = item[0]

                    my_cursor.execute("INSERT INTO image (destination_id, image_data) VALUES (%s, %s)",
                                      (d_id, image_binary_data))

                database.commit()
                messagebox.showinfo("Success", "Images have been saved")
                update_image()
        else:
            messagebox.showerror("Error", "Please select a destination before saving images")
    ...
    def _on_close(self):
        my_cursor.execute('''SELECT destination_id FROM destination''')
        all_id_t = my_cursor.fetchall()
        all_id = []
        for a in all_id_t:
            all_id.append(a[0])
        my_cursor.execute('''SELECT DISTINCT destination_id FROM image''')
        image_exists_t = my_cursor.fetchall()
        image_exists = []
        for b in image_exists_t:
            image_exists.append(b[0])
        my_cursor.execute('''SELECT destination_id FROM price''')
        with_price_t = my_cursor.fetchall()
        with_price = []
        for c in with_price_t:
            with_price.append(c[0])
        my_cursor.execute('''SELECT destination_id FROM destination WHERE destination_type='attraction' 
                          AND destination_estimated_price!=0''')
        with_ticket_t = my_cursor.fetchall()
        with_ticket = []
        for d in with_ticket_t:
            with_ticket.append(d[0])
        if sorted(all_id) != sorted(image_exists) or sorted(with_ticket) != sorted(with_price):
            if sorted(all_id) != sorted(image_exists):
                missing = set(sorted(all_id)) - set(sorted(image_exists))
                error_message = f"Please insert at least one image for destination with ID {', '.join(map(str, missing))}."
                messagebox.showerror('Error', error_message)
            elif sorted(with_ticket) != sorted(with_price):
                missing_t = set(sorted(with_ticket)) - set(sorted(with_price))
                messagebox.showerror('Error', f"Please insert the prices of tickets for destination with ID "
                                              f"{', '.join(map(str, missing_t))}.")
        else:
            self.root.withdraw()
            self.Reset()
            self.clear_frame(self.side_frame)
            self.price_window.withdraw()
...
def switch_main_window():
    ...
    if user_type:
        ...
        for widgets in transport_content_frame.winfo_children():
            widgets.destroy()
        various_transport()
        transport_canvas.yview_moveto(0.0)

        select_content_food_beverage()
        for widgets in food_beverage_main_frame.winfo_children():
            widgets.destroy()
        ...
        food_beverage_canvas.yview_moveto(0.0)

        navigate_to_spots()
        select_content_spots()
        for widgets in spots_main_frame.winfo_children():
            widgets.destroy()
        ...
        spots_canvas.yview_moveto(0.0)

        feedback_canvas.yview_moveto(0.0)
        purchase_history_canvas.yview_moveto(0.0)
        booking_history_canvas.yview_moveto(0.0)
        games_question_list_canvas.yview_moveto(0.0)
        ...
        if user_type[0] == 'user':
            ...
            profile_menu.delete('Sign Out')
            profile_menu.add_command(label='Manage Post', font=('Arial', 10), command=open_managepostsystem_window)
...
def sign_out():
    ...
    if "Manage Post" == profile_menu.entrycget(3, 'label'):
        profile_menu.delete('Manage Post')
        profile_menu.delete('View Feedback')
    clear_path()
...
def show_password3():
    if openEye_status3.cget('text') == 'Close':
...
def show_password4():
    if openEye_status4.cget('text') == 'Close':
...
def switch_to_feedback():
    ...
    feedback_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, feedback_canvas))
...
def switch_to_purchase_history():
    ...
    purchase_history_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, purchase_history_canvas))
...
def switch_to_booking_history():
    ...
    booking_history_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, booking_history_canvas))
...
def navigate_to_transport():
    ...
    transport_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, transport_canvas))
...
def navigate_to_food_beverage():
    ...
    food_beverage_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, food_beverage_canvas))
...
def navigate_to_games():
    ...
    display_questions(current_question_index)
...
def navigate_to_spots():
    ...
    spots_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, spots_canvas))
...
def open_managepostsystem_window():
    global manage_instance
    if manage_instance is not None:
        manage_instance.root.deiconify()
        manage_instance.root.lift()
    else:
        manage_instance = ManagePostSystem(main_window)
        manage_instance.run()
...
def update_image():
    global x
    image_cursor.execute('''SELECT * FROM image''')
    image_detail = image_cursor.fetchall()

    for detail in image_detail:
        if detail[2] not in all_images_data:
            image_stream1 = BytesIO(detail[2])
            img1 = Image.open(image_stream1)
            resized_img1 = img1.resize((210, 125), Image.LANCZOS)
            tk_image1 = ImageTk.PhotoImage(resized_img1)
            destination_id1 = detail[1]
            image_dict[x] = [destination_id1, tk_image1]

            r_resized_img1 = img1.resize((550, 290), Image.LANCZOS)
            r_tk_image1 = ImageTk.PhotoImage(r_resized_img1)
            r_image_dict[x] = [destination_id1, r_tk_image1]

            h_resized_img1 = img1.resize((200, 150), Image.LANCZOS)
            h_tk_image1 = ImageTk.PhotoImage(h_resized_img1)
            h_image_dict[x] = [destination_id1, h_tk_image1]
            all_images_data.append(detail[2])
            x += 1
    ...
    if num_results == 0:
        ...
        for attraction in attraction_list:
            ...
            def attraction_detail(n, r, d, p, a, c, o, t, i):
                ...
                select_attraction()
                spots_second_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, spots_second_canvas))
                spots_second_canvas.yview_moveto(0.0)
    ...
    if num_results == 0:
        ...
        for restaurant in restaurant_list:
            ...
            def restaurant_detail(n, r, d, p, a, c, o, t, i):
                ...
                select_restaurant()
                food_beverage_second_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, food_beverage_second_canvas))
                food_beverage_second_canvas.yview_moveto(0.0)
...
def display_questions_list(q):
    user_name = username1_entry.get()
    cursor.execute('SELECT user_id FROM USER where user_name=%s', (user_name,))
    user_id = cursor.fetchone()[0]
    cursor.execute('''SELECT games_id from gameshistory WHERE user_id=%s and games_status=%s''', (user_id, 1))
    history = cursor.fetchall()
    history_question_id = []
    for h in history:
        history_question_id.append(h[0])

    row_value = 1
    for i in q:
        q_id = i[0]
        if q_id in history_question_id:
            fg_value = '#FFFFFF'
        else:
            fg_value = '#7F7C7C'
        question_button = tk.Button(question_list_frame, bg='#232F34', fg=fg_value, wraplength=530, justify='left',
                                    font=('Arial', 12), border=0,
                                    command=lambda question_id=q_id: display_questions(question_id))
        question_button.grid(row=row_value, column=1, pady=8, sticky=tk.W)
        question_button.config(text=str(q_id) + '. ' + i[1])
        row_value += 1
...
def display_questions(question_id):
    ...
    games_option1_button.config(text=options[0], command=lambda: check_answer(question_id))
    games_option2_button.config(text=options[1], command=lambda: check_answer(question_id))
    games_option3_button.config(text=options[2], command=lambda: check_answer(question_id))
    games_option4_button.config(text=options[3], command=lambda: check_answer(question_id))
    ...
    if question_id in history_question_id:
...
def check_answer(question_id):
    ...
    if question_id not in history_question_id:
        query = "INSERT INTO gameshistory (user_id, games_id, games_status) " \
                "VALUES (%s, %s, %s)"
        cursor.execute(query, (user_id, question_id, 1))
        database.commit()
    else:
        pass
    ...
    for option_text, option_label in list(option_labels.items()):
...
def open_games_list():
    ...
    arrow_back_button.place_forget()
    arrow_next_button.place_forget()
    ...
    games_question_list_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, games_question_list_canvas))
...
gmaps = googlemaps.Client(key='AIzaSyCTOgO-jvFBtccx8a7JQ_mbO4YzZhlsvIs')

# Create a Geocoder instance
geocoder = Nominatim(user_agent="BWong")

# Create lists to keep track of markers and paths
markers = []
added_paths = []
...
def calculate_distance():
    start_location = start_input.get()
    end_location = end_input.get()

    coordinates1 = geocoder.geocode(start_location)
    coordinates2 = geocoder.geocode(end_location)

    try:
        if coordinates1 is None or coordinates2 is None:
            raise ValueError("Invalid location(s). Please check your input.")

        lat1, long1 = coordinates1.latitude, coordinates1.longitude
        lat2, long2 = coordinates2.latitude, coordinates2.longitude

        # Use the Directions API to get the optimized route with "driving" mode
        ...
        result_label.config(text=f"Distance: {distance_text}, Duration: {duration_text}")
        ...
        marker_start = map_widget.set_marker(lat1, long1, text=start_location)
        marker_end = map_widget.set_marker(lat2, long2, text=end_location)

        markers.extend([marker_start, marker_end])

        # Extract the steps of the route and add them to the map
        ...
        # Add the path to the map and store it in the list
        map_path = map_widget.set_path(path)
        added_paths.append(map_path)

    except googlemaps.exceptions.ApiError as e:
        if "NOT_FOUND" in str(e):
            result_label.config(text="Invalid location(s). Please check your input.")
        else:
...
# Function to add a marker for the location and calculate distance if two locations are entered
def add_marker_and_calculate_distance():
    map_frame.focus_set()
    location = start_input.get()
    ...
    if coordinates is None:
        result_label.config(text="Invalid location. Please check your input.")
        return

    lat, long = coordinates.latitude, coordinates.longitude
    marker = map_widget.set_marker(lat, long, text=location)
    markers.append(marker)

    # If both start and end locations are available, calculate the distance
    if end_input.get():
        calculate_distance()
...
def clear_paths():
    for path in added_paths:
        map_widget.delete(path)
    added_paths.clear()


# Function to clear both markers and paths
def clear_path():
    clear_markers()
    clear_paths()
    clear_input_fields()
    start_input.config(foreground="grey")
    start_input.insert(0, "Start")
    end_input.config(foreground="grey")
    end_input.insert(0, "End")
    map_frame.focus_set()
...
def clear_input_fields():
    ...
    end_input.delete(0, tk.END)


# Function to clear placeholder text when clicked
def clear_placeholder(event):
    if start_input.get() == "Start":
        start_input.delete(0, "end")
        start_input.config(foreground="black")  # Change text color to black

    if end_input.get() == "End":
        end_input.delete(0, "end")
        end_input.config(foreground="black")  # Change text color to black
...
# Load your rectangle3 image
rectangle3_image = PhotoImage(file="C:/Users/xueer/OneDrive/Desktop/ALL2 (4007CEM)/Rectangle 30.png")

# Create Rectangle 3 with the loaded image
rectangle3 = tk.Canvas(map_frame, width=364, height=350, bg="#344955", highlightthickness=0)
rectangle3.create_image(0, 0, anchor=tk.NW, image=rectangle3_image)
rectangle3.place(x=34, y=100)

# Load your rectangle4 image
rectangle4_image = PhotoImage(file="C:/Users/xueer/OneDrive/Desktop/ALL2 (4007CEM)/Rectangle 31.png")

# Create Rectangle 4 with the loaded image
rectangle4 = tk.Canvas(map_frame, width=364, height=272, bg="#D9D9D9", highlightthickness=0)
rectangle4.create_image(0, 0, anchor=tk.NW, image=rectangle4_image)
rectangle4.place(x=34, y=179)

# Load your get_somewhere_label image
get_somewhere_label_image = PhotoImage(file="C:/Users/xueer/OneDrive/Desktop/ALL2 (4007CEM)/GET ME SOMEWHERE.png")

# Create get_somewhere_label with the loaded image
get_somewhere_label = tk.Label(map_frame, image=get_somewhere_label_image, bg="#344955")
get_somewhere_label.place(x=34, y=118, width=364, height=44)

# Create the search button
search_button = tk.Button(map_frame, text="GO", bg="#F9AA33", font=("Montserrat", 24, "bold",))
search_button.place(x=272, y=350, width=100, height=50)  # Adjust the position and size
search_button.config(command=add_marker_and_calculate_distance)

# Create the clear button with the loaded and resized image
clear_button = tk.Button(map_frame, text="Clear", bg="#F9AA33", font=("Montserrat", 24, "bold"))
clear_button.place(x=57, y=350, width=100, height=50)  # Adjust the position and size
clear_button.config(command=clear_path)

# Create a frame to embed Google Maps with the specified dimensions and position
frame_map = tk.Frame(map_frame, width=495, height=350, bg="#344955")
frame_map.place(x=420, y=100)

# Create a Google Maps widget and embed it inside the frame with the specified dimensions and corner_radius
map_widget = TkinterMapView(frame_map, width=495, height=350, corner_radius=25)
map_widget.pack(fill="both", expand=True)  # This line resizes the widget to fill the available space in the frame

# Configure Google Maps with your desired settings
map_widget.set_tile_server("https://mt0.google.com/vt/lyrs=m&hl=en&x={x}&y={y}&z={z}&s=Ga", max_zoom=22)
map_widget.set_address("Penang", marker=False)

# Add a blue marker for the specified address during initialization
map_widget.set_marker(5.4189341, 100.3367176, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4760004, 100.2523773, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4146247, 100.3385884, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4201056, 100.3300552, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4184610, 100.3353534, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4247878, 100.3206162, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4177184, 100.3370613, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4658397, 100.2895162, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4527664, 100.3063889, marker_color_circle="white", marker_color_outside="blue")
map_widget.set_marker(5.4122058, 100.3281193, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4130183, 100.3397572, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.3997235, 100.2738880, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4198380, 100.3417777, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4141784, 100.3304024, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4476629, 100.2152795, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4162422, 100.3412181, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4154655, 100.3367970, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4498635, 100.2140979, marker_color_circle="white", marker_color_outside="purple")
map_widget.set_marker(5.4255773, 100.2679884, marker_color_circle="white", marker_color_outside="blue")

# Pack the map widget
map_widget.pack(fill="both", expand=True)

# Create a label to display the distance result
result_label = tk.Label(map_frame, text="", bg="#D9D9D9", fg="black")
result_label.place(x=50, y=420, width=344, height=30)  # Adjust the position and size

# Create "Start" input field
start_input = ttk.Entry(map_frame, font=("Montserrat", 24, "bold"), style="TEntry")
start_input.insert(0, "Start")  # Set the placeholder text
start_input.config(foreground="grey", style="TEntry", background="#D9D9D9")
start_input.place(x=57, y=215, width=315, height=50)
start_input.bind("<FocusIn>", clear_placeholder)  # Bind the event to clear placeholder text

# Create "End" input field
end_input = ttk.Entry(map_frame, font=("Montserrat", 24, "bold"), style="TEntry")
end_input.insert(0, "End")  # Set the placeholder text
end_input.config(foreground="grey", style="TEntry", background="#D9D9D9")  # Set text color to grey and background color
end_input.place(x=57, y=280, width=315, height=50)
end_input.bind("<FocusIn>", clear_placeholder)  # Bind the event to clear placeholder text
...
def create_feedback_form():
    ...
    def submit_form():
        ...
        if count_true == 1:
            ...
            messagebox.showerror('Error', 'Please select ONE type for this feedback form')
...
def all_feedback():
    ...
    def select_feedback(selection):
        ...
        view_feedback_canvas.bind_all("<MouseWheel>", lambda event: on_mouse_wheel(event, view_feedback_canvas))
        view_feedback_canvas.yview_moveto(0.0)
