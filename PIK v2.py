...
def update_ph_bh():
    cursor.execute('''SELECT purchase_booking_date, purchase_booking_status, purchase_booking_id FROM purchase_booking''')
    ph_bh = cursor.fetchall()
    for history in ph_bh:
        target_date = history[0]
        status = history[1]
        ph_bh_id = history[2]
        if status == 'Ready':
            current_datetime = datetime.datetime.now()
